...
##########################################################################
echo -e "";
read -sp "1. Enter a password for your Postgres Database (If left blank, it will be autogenerated): " POSTGRES_PASSWORD

##########################################################################
# JWT Secret (Will be used to sign Anon Role and Service Role keys)
##########################################################################
echo -e "";
read -sp "2. Enter a JWT Secret to be used by Supabase (If left blank, it will be autogenerated): " JWT_SECRET

##########################################################################
# SUPABASE URL (STUDIO, POSTGRES, KONG ETC)
##########################################################################
echo -e "";
read -p "$(echo -e "3. Enter the domain you will use to access Supabase Studio ${RED}[Required]${NC}: https://")" domain
if [ -z "$domain" ]
then
    while [[ -z "$domain" ]]; do
...
# EMAIL SIGN UPS
##########################################################################
read -p "$(echo -e "4. Do you wish to enable Email Signups? ${RED}[You will require a custom SMTP Server]${NC} [y/n]: ")" enable_email_signup
